# res://addons/godot-stat-math/tests/performance/ppf_functions_perf_test.gd
class_name PpfFunctionsPerfTest extends GdUnitTestSuite
...
func _check_performance_regression(test_name: String, current_results: Dictionary, baseline_data: Dictionary) -> void:
	...
	var prefixed_test_name: String = "ppffunctions_%s" % test_name
	...
	TestRunCollector.add_test_result("PpfFunctions", test_name, current_time, baseline_time, is_failure)
...
func collect_performance_measurements() -> Dictionary:
	...
	print("Measuring PPF functions performance...")
	
	# Normal PPF tests
	var measurement: Dictionary = _measure_test("normal_ppf", func():
		for i in range(TEST_ITERATIONS):
			for p_val in PROBABILITY_VALUES:
				StatMath.PpfFunctions.normal_ppf(p_val, 0.0, 1.0)
				StatMath.PpfFunctions.normal_ppf(p_val, 5.0, 2.0)
	)
	results["normal_ppf"] = measurement.execution_time_ms
	print("  normal_ppf: %.2f ms" % measurement.execution_time_ms)
	
	# Gamma PPF tests
	measurement = _measure_test("gamma_ppf", func():
		for i in range(TEST_ITERATIONS):
			for p_val in PROBABILITY_VALUES:
				StatMath.PpfFunctions.gamma_ppf(p_val, 2.0, 1.0)
				StatMath.PpfFunctions.gamma_ppf(p_val, 5.0, 0.5)
	)
	results["gamma_ppf"] = measurement.execution_time_ms
	print("  gamma_ppf: %.2f ms" % measurement.execution_time_ms)
	
	# Beta PPF tests
	measurement = _measure_test("beta_ppf", func():
		for i in range(TEST_ITERATIONS):
			for p_val in PROBABILITY_VALUES:
				StatMath.PpfFunctions.beta_ppf(p_val, 2.0, 3.0)
				StatMath.PpfFunctions.beta_ppf(p_val, 0.5, 0.5)
	)
	results["beta_ppf"] = measurement.execution_time_ms
	print("  beta_ppf: %.2f ms" % measurement.execution_time_ms)
	
	# Weibull PPF tests
	measurement = _measure_test("weibull_ppf", func():
		for i in range(TEST_ITERATIONS):
			for p_val in PROBABILITY_VALUES:
				StatMath.PpfFunctions.weibull_ppf(p_val, 2.0, 1.5)
				StatMath.PpfFunctions.weibull_ppf(p_val, 1.0, 2.0)
	)
	results["weibull_ppf"] = measurement.execution_time_ms
	print("  weibull_ppf: %.2f ms" % measurement.execution_time_ms)
