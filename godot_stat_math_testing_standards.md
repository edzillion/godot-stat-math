...
All data tables in `/addons/godot-stat-math/tables/` must follow these strict conventions:

**✅ CORRECT Format** (Plain Script):
```gdscript
# res://addons/godot-stat-math/tables/example_data.gd

## Brief description of the data table's purpose
##
...
**❌ INCORRECT Format** (Class Declaration):
```gdscript
# res://addons/godot-stat-math/tables/example_data.gd
class_name ExampleData  # ← NEVER DO THIS
...
- Include scipy version information in header comments
- Use `const VALUES: Dictionary` for the main data structure
- Follow the same pattern as `sobol_data.gd` and `prime_numbers_data.gd`
...
**Important**: Data tables are **plain scripts**, not classes. They should not include `class_name` declarations. This follows the same pattern as existing tables like `sobol_data.gd` and `prime_numbers_data.gd`.
